...
def api_calculadora_motorista(dados: MotoristaRequest):
    ...
    elif validar_consumo_veiculo(dados.consumo_veiculo) is not True:
        raise HTTPException(status_code=400, detail="Valores inválidos.")
    elif validar_valor_combustivel(dados.valor_combustivel) is not True:
        raise HTTPException(status_code=400, detail="Valores inválidos.")
    elif validar_ganhos(dados.ganhos) is not True:
        raise HTTPException(status_code=400, detail="Valores inválidos.")
    elif validar_alimentacao(dados.alimentacao) is not True:
        raise HTTPException(status_code=400, detail="Valores inválidos.")
    elif validar_cafe(dados.cafe) is not True:
        raise HTTPException(status_code=400, detail="Valores inválidos.")
    elif validar_outros_gastos(dados.outros_gastos) is not True:
        raise HTTPException(status_code=400, detail="Valores inválidos.")
    else:
